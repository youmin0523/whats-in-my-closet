...
const PRESETS: { key: string; label: string; sections: CellType[][] }[] = [
  { key: "single", label: "단문형", sections: [["rod", "shelf", "shelf", "drawer"]] },
  {
    key: "double",
    label: "양문형",
    sections: [
  ...
  },
  {
    key: "triple",
    label: "3열장",
    sections: [["rod", "shelf"], ["shelf", "shelf", "shelf"], ["rod", "drawer", "drawer"]],
  },
  { key: "drawers", label: "서랍장", sections: [["drawer", "drawer", "drawer", "drawer"]] },
  { key: "rodshelf", label: "행거+선반", sections: [["rod", "shelf", "shelf", "shelf"]] },
];

export function ClosetBuilder() {
  const [name, setName] = useState("");
  const [sections, setSections] = useState<CellType[][]>(
    PRESETS[1]!.sections.map((c) => [...c]),
  );

  const addCell = (col: number, type: CellType) =>
    setSections((s) => s.map((c, i) => (i === col ? [...c, type] : c)));
  const removeCell = (col: number, row: number) =>
    setSections((s) =>
      s.map((c, i) => (i === col ? c.filter((_, r) => r !== row) : c)),
    );
  const addCol = () => setSections((s) => [...s, ["shelf"]]);
  const removeCol = (col: number) =>
    setSections((s) => (s.length > 1 ? s.filter((_, i) => i !== col) : s));

  const total = sections.reduce((n, c) => n + c.length, 0);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {PRESETS.map((p) => (
            <button
              key={p.key}
              type="button"
              onClick={() => setSections(p.sections.map((c) => [...c]))}
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          {sections.map((cells, col) => (
            <div
              key={col}
              className="flex min-w-[128px] flex-1 flex-col gap-2 rounded-lg border bg-background/60 p-2"
            >
      ...
      <form action={buildClosetAction} className="flex items-center justify-between gap-3">
        <input type="hidden" name="name" value={name} />
        <input
          type="hidden"
          name="sections"
          value={JSON.stringify(sections.map((c) => c.map((type) => ({ type }))))}
        />
...
function Cell({ type, onRemove }: { type: CellType; onRemove: () => void }) {
  return (
    <div className="relative flex h-12 items-center justify-center rounded-md border bg-secondary/40 text-[11px] text-muted-foreground">
      {type === "rod" && (
      ...
      <button
        ...
        onClick={onRemove}
        aria-label="삭제"
        className="absolute right-1 top-0.5 text-sm leading-none text-muted-foreground hover:text-destructive"
      >
        ×
      </button>
    </div>
